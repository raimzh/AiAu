...
### [2026-08-28] — Оптимизация изображений

#### Изменено
- Убран `images.unoptimized` — фото отдавались исходными JPEG всем устройствам. Теперь Next отдаёт AVIF/WebP нужного размера: замеры на 640px — `bracelet-001` 102 → 15 КБ (−86%), `ring-001` 29 → 8 КБ (−72%), `earrings-003` 77 → 33 КБ (−56%)
- В карточке товара добавлен `sizes` — браузер качает вариант под свою колонку, а не самый большой
- Запасной `placeholder.svg` при битом пути к фото по-прежнему работает: оптимизатор отдаёт 400, `onError` подставляет заглушку
